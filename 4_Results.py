...
if not st.session_state.get("run_results"):
    st.warning("⚠️ No results yet. Go to Run Pipeline first.")
...
col5.metric("High-priority gaps (>=60)", f"{gap_high:,}")
...
fc = st.columns(4)
for i, (freq, icon) in enumerate([("weekly","🟢"), ("fortnightly","🔵"), ("monthly","🟡"), ("bi-weekly","🟠")]):
    fc[i].metric(f"{icon} {freq.title()}", f"{freq_counts.get(freq, 0):,} stores")
...
if filtered:
    ...
    st.dataframe(
        df_show, use_container_width=True, height=380,
        column_config={
...
# ── Gap report ────────────────────────────────────────────────────────────────
st.subheader("Top gap opportunities (score >= 40)")
...
if high_gaps:
    ...
    st.info("No gap stores with score >= 40 found.")
...
for s in all_stores:
    ...
    if rid not in rep_data:
        rep_data[rid] = {"Rep ID": rid, "Stores": 0, "Calls / Month": 0.0}
    rep_data[rid]["Stores"]        += 1
    rep_data[rid]["Calls / Month"] += s.get("calls_per_month", 0)
if rep_data:
    rdf = pd.DataFrame(list(rep_data.values())).sort_values("Rep ID")
    rdf["Calls / Month"] = rdf["Calls / Month"].round(1)
...
with col3:
    ...
    st.download_button("🗺️ Routes GeoJSON",

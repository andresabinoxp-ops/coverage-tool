...
if st.button("  Run Coverage Agent", type="primary"):
    ...
    # ── LIVE RUN ──────────────────────────────────────────────────────────────
        ...
        if enrich_scope != "none":
            ...
            # ── Skip stores already enriched — avoids redundant API calls ──
            # A store already has phone & hours if phone is non-empty.
            # This cuts Stage 7 time by 80-90% when cache was used in Step 2.
            candidates = [s for s in candidates if not s.get("phone","")]
            already_enriched = enrich_count - len(candidates)
            if already_enriched > 0:
                status.info(
                    f"Stage 7/{total_steps} — {already_enriched} stores already have "
                    f"phone & hours (from cache) — enriching remaining {len(candidates)} only."
                )
        ...
        if enrich_poi != "none":
            ...
            # ── Skip stores that already have POI count from cache ──
            poi_already = sum(1 for s in poi_candidates if s.get("poi_count", 0) > 0)
            poi_candidates = [s for s in poi_candidates if not s.get("poi_count", 0)]
            if poi_already > 0:
                status.info(
                    f"Stage {poi_stage}/{total_steps} — {poi_already} stores already have "
                    f"POI count (from cache) — enriching remaining {len(poi_candidates)} only."
                )

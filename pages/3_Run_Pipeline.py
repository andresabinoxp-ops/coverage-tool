...
if st.button("  Run Coverage Agent", type="primary"):
    ...
    # ── LIVE RUN ──────────────────────────────────────────────────────────────
        ...
        # Stage 2: Universe — use cache if available, only scrape if not cached
        _run_cache_key2 = _cache_key  # reuse the key already computed at top of page
        _run_cache2     = st.session_state.get("universe_cache", {}).get(_run_cache_key2)

        if _run_cache2:
            universe = [dict(s) for s in _run_cache2["universe"]]
            status.info(
                f"Stage 2/{total_steps} — Using cached universe: "
                f"{len(universe):,} stores (scraped {_run_cache2.get('scraped_at','')}) — skipping API scrape."
            )
            bar.progress(45)
        else:
            ...
            # Auto-save to cache so next run doesn't re-scrape
            if "universe_cache" not in st.session_state:
                st.session_state["universe_cache"] = {}
            st.session_state["universe_cache"][_run_cache_key2] = {
            ...
            status.info(f"Stage 2/{total_steps} — Found {len(universe):,} unique stores (auto-saved to cache)")
            bar.progress(45)
        ...
        if enrich_scope != "none":
            ...
            for i, store in enumerate(candidates):
                ...
                try:
                    ...
                    _det_r = requests.get(PLACE_DETAIL_URL,
                        params={"place_id": store.get("place_id",""),
                                "fields": "formatted_phone_number,opening_hours,website,formatted_address",
                                "key": api_key},
                        timeout=10)
                    _det_data = _det_r.json()
                    if _det_data.get("status") == "REQUEST_DENIED":
                        status.warning(
                            "  Place Details API not enabled — skipping phone enrichment. "
                            "Enable it in Google Cloud Console → APIs & Services → Library → "
                            "Places API → Enable. Pipeline will continue without phone data."
                        )
                        _enrich_denied = True
                        break
                    result = _det_data.get("result", {}) if _det_data.get("status") == "OK" else {}
                    if result:

...
if st.button("Run Coverage Agent", type="primary"):
    ...
    if dry_run:
        ...
        st.session_state["run_results"] = {
            ...
            "coverage_rate_after":  round(sum(1 for s in all_stores if s["covered"])/max(len(all_stores),1)*100, 1),
        ...
        status.info(f"Stage 2 — Geocoding {len(portfolio)} stores...")
        bar.progress(10)
        for s in portfolio:
        ...
        status.info("Stage 3 — Scraping Google Places...")
                ...
                while True:
                    ...
                    for place in data.get("results", []):
                        ...
                        universe.append({
                            ...
                            "address":place.get("vicinity",""),"city":cfg["market_name"],
        ...
        status.info(f"Stage 4 — Scoring {len(portfolio)+len(universe)} stores...")
        ...
        status.info("Stage 6 — Assigning frequencies...")
        for s in all_stores:
        ...
        status.info("Stage 7 — Allocating routes...")

...
if colour_by == "Rep route":
    ...
    if reps:
        rep_cols = st.columns(min(len(reps), 5))
        for i, rep in enumerate(reps):
            ...
            rep_cols[i%5].markdown(
                f'<div style="background:{color_hex};padding:8px;border-radius:6px;color:white;text-align:center">'
    ...
    lc = st.columns(2)
    lc[0].markdown('<div style="background:#00c882;padding:8px;border-radius:6px;color:white;text-align:center"><b>Covered</b><br>In current portfolio</div>', unsafe_allow_html=True)
...
if all_reps:
    ...
    if sel != "All reps":
        ...
        if rep_stores:
            ...
            show = [c for c in ["store_name","category","score","visit_frequency","calls_per_month","coverage_status","address"] if c in rdf.columns]

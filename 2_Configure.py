...
with col2:
    st.markdown("**Google API key override (optional)**")
    market_api_key = st.text_input(
        "Market-specific Google API key",
        type="password",
        placeholder="Leave blank to use the global key set by admin",
        help="Only needed if this market uses a different Google Cloud billing account.",
    )
...
selected_cats = st.multiselect(
    "Select categories for Google Places scraping",
...
st.caption("Tier 1 = highest FMCG fit (1.0 multiplier) down to Tier 4 (0.30 multiplier)")
...
if selected_cats:
    ...
    st.warning("Select at least one category above.")
...
col1, col2 = st.columns(2)
with col1:
    w_rating   = st.slider("Rating — Google Places 0-5",          0, 50, pv("w_rating",   20))
    w_reviews  = st.slider("Reviews / footfall — log-normalised", 0, 50, pv("w_reviews",  25))
    w_category = st.slider("Category fit — tier multiplier",      0, 50, pv("w_category", 20))
with col2:
    w_sales    = st.slider("Current sales — covered stores only", 0, 50, pv("w_sales",    20))
    w_lines    = st.slider("Lines per store — SKU breadth",       0, 50, pv("w_lines",    15))

total = w_rating + w_reviews + w_category + w_sales + w_lines
if total == 100:
    st.success(f"✅ Total: {total}% — weights are valid")
else:
    st.error(f"❌ Total: {total}% — must equal exactly 100% (currently {'+' if total > 100 else ''}{total - 100}%)")
...
if total != 100:
    st.warning("Fix the scoring weights to equal 100% before saving.")
elif not selected_cats:
    st.warning("Select at least one category before saving.")
else:
    if st.button("💾 Save market configuration", type="primary"):
        ...
        st.success(f"✅ Configuration saved for **{market_name}**. Go to Run Pipeline to upload your portfolio.")

# ── Show saved config ─────────────────────────────────────────────────────────
if st.session_state.get("market_config"):
    with st.expander("View saved configuration"):
        cfg_display = dict(st.session_state["market_config"])
        cfg_display.pop("market_api_key", None)
        st.json(cfg_display)
